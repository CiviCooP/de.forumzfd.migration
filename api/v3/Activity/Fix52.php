...
function civicrm_api3_activity_Fix52($params) {
  $civiTable = "civicrm_value_kampagnen_f_r_unterschriftenliste_52";
  $fzfdTable = "forumzfd_value_kampagnen_f_r_unterschriftenliste_52";
  set_time_limit(0);
  $countCreated = 0;
  $countLogged = 0;
  $returnValues = array();
  ...
  $logger = new CRM_Migration_Logger('kampagnen_52');
  ...
  $returnValues[] = 'All kampagnen_für_unterschriftenliste_52 custom data for activities migrated to CiviCRM';
  return civicrm_api3_create_success($returnValues, $params, 'Activity', 'Fix52');
}
...
function _createQueryAndParams($civiTable, $daoSource, &$insertQuery, &$insertParams) {
  ...
  if (!empty($daoSource->unterschriftenliste_470)) {
    $insertIndex++;
    $insertFields[] = 'unterschriftenliste_470';
    $insertValues[] = '%'.$insertIndex;
    $insertParams[$insertIndex] = array($daoSource->unterschriftenliste_470, 'String');
  }
...
function _fixExtendValues($civiTable) {
  $newActivityTypeId = CRM_Core_DAO::singleValueQuery('SELECT value FROM civicrm_option_value WHERE option_group_id = 2
    AND name = "An Petition teilgenommen"');
