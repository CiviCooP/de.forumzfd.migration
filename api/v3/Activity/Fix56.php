...
function civicrm_api3_activity_Fix56($params) {
  $civiTable = "civicrm_value_felder_f_r_bestellungsaktivit_ten_56";
  $fzfdTable = "forumzfd_value_felder_f_r_bestellungsaktivit_ten_56";
  set_time_limit(0);
  $countCreated = 0;
  $countLogged = 0;
  $returnValues = array();
  ...
  $logger = new CRM_Migration_Logger('bestellung_56');
  ...
  $returnValues[] = 'All bestellung_56 custom data for activities migrated to CiviCRM';
  return civicrm_api3_create_success($returnValues, $params, 'Activity', 'Fix56');
}
...
function _createQueryAndParams($civiTable, $daoSource, &$insertQuery, &$insertParams) {
  ...
  if (!empty($daoSource->produkt_das_bestellt_wurde_500)) {
    $insertIndex++;
    $insertFields[] = 'produkt_das_bestellt_wurde_500';
    $insertValues[] = '%'.$insertIndex;
    $insertParams[$insertIndex] = array($daoSource->produkt_das_bestellt_wurde_500, 'String');
  }
